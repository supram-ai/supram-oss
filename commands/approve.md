...
notes: |
  Human Gate 1. After /supram:define produces the spec (spec.yaml, with design and tasks
  internalised), the human explicitly approves it before /supram:build may run.

gates:
  - check: 'spec.yaml exists'
    on_fail: STOP, run /supram:define
