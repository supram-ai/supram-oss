...
gates:
  - check: spec.yaml exists (to read workflow level and constraints)
    on_fail: STOP, run /supram:define first

actions:
  - classify_change: identify if the change is a bug or CR
  - evaluate_promotion_criteria: promote to full /supram:define if the change touches a locked stack, public interface, persistent schema, security boundary, or architecture boundary
