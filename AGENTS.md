...
|---------|-----------------|-----------|---------|
| `/supram:init` | Manager | N/A — bootstrap | scaffold + constitution |
| `/supram:define` | Analyst | Reads BRD, writes spec.yaml (design + tasks internal) | spec.yaml |
| `/supram:approve` | Gatekeeper (human gate) | N/A — waits for human | Ref: APPROVED |
| `/supram:build` | Developer | Reads spec.yaml, writes code | code + evidence |
| `/supram:review-pre-verify` | Sr Tech Lead (L-level only) | Fresh context, no build memory | review report |
| `/supram:verify` | Gatekeeper | Runs evidence, writes report | verify/slice.yaml |
| `/supram:release` | Gatekeeper (human gate) | N/A — waits for human | merged PR |
| `/supram:change` | Developer | Reads change context, writes delta | CHG record + code |
| `/supram:status` | Manager | N/A — read only | status snapshot |

Design and tasks are internalised into `/supram:define`; there are no separate design, tasks, or
...
Manager receives request
  ↓
Manager checks spec approval: spec.yaml Ref: APPROVED? (else route to /supram:approve — Human Gate 1)
  ↓
Manager spawns /supram:build subagent (Developer persona)
  ↓
...
Manager checks gates (all tasks complete? tests pass?)
  ↓
IF workflow_level == L: Manager spawns /supram:review-pre-verify (Sr Tech Lead, isolated)
  ↓
Subagent reviews with fresh eyes → returns control to Manager
  ↓
Manager spawns /supram:verify subagent (Gatekeeper)
  ↓
Subagent runs tests, writes verify/slice.yaml → returns control
  ↓
Manager checks Release Ref: PENDING
  ↓
Manager waits for human approval (/supram:release — Human Gate 2)
```
...
```
spec.yaml Ref: PENDING → /supram:approve (Human Gate 1) → Ref: APPROVED
  ↓
build checks spec Ref: APPROVED (hard gate)
  ↓
[L-level only] review-pre-verify Ref: PENDING (Sr Tech Lead subagent) → APPROVED
  ↓
verify/slice.yaml Release Ref: PENDING → /supram:release (Human Gate 2) → APPROVED
```
...
On first use, run `/supram:init` or say "initialise this project using supram-oss".
...
- Use `/supram:init` clarification answers only as initialization input.
...
2. **NEVER start implementing before the spec is human-approved via `/supram:approve`** (Human Gate 1) — except for S-level projects which skip the design and tasks phase. If `workflow_level` is absent, default to M/L (full gates).
...
|---------|-------------|------|
| `/supram:define` | Create slice contract (spec.yaml) from BRD. Design/tasks are internalised. | — |
| `/supram:build` | Implement against approved Evidence Contract — one commit per task | — |
| `/supram:verify` | Run tests, check acceptance criteria, fill verify/slice.yaml | — |
| `/supram:change` | Unified bug and CR workflow: baseline → implement smallest delta → verify | — |
| `/supram:status` | Read-only project status snapshot — no tests, no network, no mutations | — |
 
### Infrequent and Internal Commands
 
| Command | What happens | Gate |
|---------|-------------|------|
| `/supram:init` | Scan docs, derive project.yaml, plan.yaml, constitution | — |
| `/supram:upgrade` | Refresh the protocol reference; hand state to the paid product | — |
...
- **Runs**: Operational commands (`/supram:init`, `/supram:upgrade`, `/supram:status`)
- **Responsibilities**:
...
|--------------|------------------|-----------------------------|--------------------------|
| **Manager** | `/supram:init`, `/supram:upgrade`, `/supram:status` | *None (Parent Context)* | Orchestrates the workflow execution, manages the subagent invocation loop, and checks status/quality gates. Run directly in the main/parent shell. |
| **Analyst** | `/supram:define` | `agents/collaborator/agent.md` | Explores the problem space and drafts the unified feature specification (`spec.yaml`, including design and tasks). |
| **Developer** | `/supram:build`, `/supram:change` | `agents/developer/agent.md` | Implements against the approved Evidence Contract and executes the unified bug/CR change workflow. |
| **Sr Tech Lead** | `/supram:review-pre-verify` (L only) | `agents/sr-tech-lead/agent.md` | Audits implementation against the approved spec, verifying alignment and conformance. |
| **Gatekeeper** | `/supram:approve`, `/supram:verify`, `/supram:release` | `agents/gatekeeper/agent.md` | Validates gate prerequisites, runs evidence validation, and transmits explicit human approvals at Gates 1 and 2. |


### Human and System Authority
- **Human**: owns both gate decisions (`/supram:approve` before build, `/supram:release` after verify).
...
2. Manager spawns subagent with correct persona (e.g., Developer for /supram:build)
