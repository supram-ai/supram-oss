...
```text
/supram:init → /supram:define → [Human Gate 1: /supram:approve] → /supram:build
        → [/supram:review-pre-verify — L only] → /supram:verify → [Human Gate 2: /supram:release]
```

Design and tasks are internalised by `/supram:define` into a single `spec.yaml`; there are no separate
...
|---------|--------------|-------------|------|
| `/supram:init` | Manager | Scan docs, derive constitution/BRD/architecture, plan phases | — |
| `/supram:define` | Analyst | Create `spec.yaml` from the BRD, including design and tasks | — |
| `/supram:approve` | Gatekeeper | **Human Gate 1** — approve the spec/design before build | Human |
| `/supram:build` | Developer | Implement against the approved Evidence Contract — one commit per task | — |
| `/supram:review-pre-verify` | Sr Tech Lead | Fresh-context review of build against the contract (**L only**) | Agent review |
| `/supram:verify` | Gatekeeper | Run approved evidence, check acceptance criteria, write verification | — |
| `/supram:release` | Gatekeeper | **Human Gate 2** — disclose deferred, merge, archive, update status | Human |
| `/supram:change` | Developer | Unified bug and CR workflow: baseline → smallest delta → verify | — |
| `/supram:status` | Manager | Project state snapshot (provided by the Supram engine) | — |
| `/supram:upgrade` | Manager | Refresh the protocol reference; hand state to the paid product | — |

> Command names and the `/supram:` prefix are shared with the Supram engine and remain stable.
...
1. **Human Gate 1 — `/supram:approve`** (before build): the human approves `Ref: APPROVED` on the
   spec/design produced by `/supram:define`. Nothing may be implemented before this.
2. **Human Gate 2 — `/supram:release`** (after verify): the human approves `Release Ref: APPROVED`,
   deferred items are disclosed, then the work is merged. Cannot proceed without explicit approval.

**Agent review (not a human gate) — `/supram:review-pre-verify`**: a Sr Tech Lead fresh-context review
...
```
main                               ← protected
  ├── feature/<FID>-<slug>        ← /supram:define
  └── change/CHG-NNN-<slug>       ← /supram:change
```
