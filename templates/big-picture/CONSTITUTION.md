...
> Design and tasks are internalised into `/supram:define`. Human Gate 1 is `/supram:approve`; the
> L-level-only agent review is `/supram:review-pre-verify`.
...
| **Developer** | `/supram:build`, `/supram:change` | `agents/developer/agent.md` | Implements against the approved Evidence Contract. |
| **Sr Tech Lead** | `/supram:review-pre-verify` (L only) | `agents/sr-tech-lead/agent.md` | Audits implementation code against the approved spec, verifying alignment and syntax conformance. |
| **Gatekeeper** | `/supram:approve`, `/supram:verify`, `/supram:release` | `agents/gatekeeper/agent.md` | Validates gate prerequisites, runs testing validation, and transmits explicit human approvals at Gates 1 and 2. |
